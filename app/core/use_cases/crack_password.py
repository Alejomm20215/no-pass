...
class CrackPasswordUseCase:
    ...
    def execute(
        ...
        if not pdf_path.exists():
            return CrackResult(success=False, error=f"File not found: {pdf_path}")

        # Check if PDF is protected
        try:
            is_protected = self.pdf_handler.is_protected(pdf_path)
            if not is_protected:
                return CrackResult(success=False, error="PDF is not password protected")
            ...
            if progress_callback:
                progress_callback(
                    0.0, f"Warning: Could not determine protection status: {e}"
                )
        ...
        if options.mode == AttackMode.DICTIONARY:
            ...
            return CrackResult(
                success=False, error="AI attack is not available in this build"
            )
        else:
            return CrackResult(
                success=False, error=f"Unsupported attack mode: {options.mode}"
            )
