...
from app.api.v1.schemas.crack import (
    CrackRequest,
    CrackResponse,
    UnlockRequest,
    UnlockResponse,
...
async def unlock_pdf(
    ...
    return UnlockResponse(success=True, output_filename=output_path.name)
...
async def crack_and_unlock(
    ...
    try:
        unlock_result = unlock_use_case.execute(
            pdf_path, str(crack_result.password), output_path
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
