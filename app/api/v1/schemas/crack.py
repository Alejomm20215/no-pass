...
from app.core.domain.entities import AttackMode, CharsetType
...
class CrackRequest(BaseModel):
    ...
    charset: CharsetType = Field(
        CharsetType.NUMERIC, description="Character set for brute force"
    )
    ...
    wordlist_file: Optional[str] = Field(
        None, description="Path to wordlist file (server-side)"
    )
    john_binary: Optional[str] = Field(
        None, description="Path to John the Ripper binary"
    )
...
class UnlockRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"password": "123456"}}
...
class UnlockResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"success": True, "output_filename": "document_unlocked.pdf"}
        }
