...
# Note: keep imports minimal; avoid unused imports that break lint rules
...
Examples:
  ...
  python cli_crack.py --file protected.pdf --mode john --john-binary /usr/bin/john
    ...
    input_group.add_argument(
        "--file", "-f", type=str, help="Individual PDF file to process"
    )
    input_group.add_argument(
        "--directory", "-d", type=str, help="Directory with PDF files"
    )
    ...
    parser.add_argument(
        "--john-binary", type=str, default="john", help="Path to John the Ripper binary"
    )
    parser.add_argument(
        "--pdf2john-binary",
        type=str,
        default="pdf2john.pl",
        help="Path to pdf2john.pl script",
    )
    parser.add_argument(
        "--pdfcrack-binary",
        type=str,
        default="pdfcrack",
        help="Path to pdfcrack binary",
    )

    args = parser.parse_args()

    # Get dependencies
    # pdf_handler is not directly used; obtain only the ones needed below
    ...
    try:
        if args.file:
            ...
            logger.info(
                f"Starting password crack on {pdf_path.name} using {args.mode} mode"
            )
            result = crack_use_case.execute(
                pdf_path, options, progress_callback if args.verbose else None
            )
                ...
                if args.save:
                    ...
                    try:
                        with open(result_file, "w", encoding="utf-8") as f:
                            f.write(
                                f"Password for {pdf_path.name}: {result.password}\n"
                            )
            ...
            logger.info(
                f"Starting batch processing of {len(list(directory.glob('*.pdf')))} PDF files"
            )
            ...
            not_protected = sum(
                1 for r in results.values() if r.get("status") == "not_protected"
            )
